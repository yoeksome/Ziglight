...
extern int32 gizPutData(uint8 *buf, uint32 len);//BY LIU
/***************************************************************************************************
 *                                         GLOBAL VARIABLES
 ***************************************************************************************************/
...
//  uartConfig.callBackFunc         = MT_UartProcessZToolData;
  uartConfig.callBackFunc         = zigLight_uartProcess; //by liu
...
*
*******************************************************/
void zigLight_uartProcess ( uint8 port, uint8 event )
{
#define TPBUF_LEN 10
  uint8  bytesInRxBuffer;
  uint8  tpbuf[TPBUF_LEN];
  (void)event;  // Intentionally unreferenced parameter
  bytesInRxBuffer = Hal_UART_RxBufLen(port);
  while (bytesInRxBuffer)
  {
    if(bytesInRxBuffer>TPBUF_LEN){
      HalUARTRead (port, tpbuf, TPBUF_LEN);
      gizPutData(tpbuf,TPBUF_LEN);
      bytesInRxBuffer -= TPBUF_LEN;
    }
    else{
      HalUARTRead (port, tpbuf, bytesInRxBuffer);
      gizPutData(tpbuf,bytesInRxBuffer);
      bytesInRxBuffer = 0;
    }
  }
}
    ...
    switch (state)
    {
      ...
      case DATA_STATE:
        ...
        if (bytesInRxBuffer <= LEN_Token - tempDataLen)
        {
          ...
          gizPutData(&pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen],bytesInRxBuffer);
          tempDataLen += bytesInRxBuffer;
        }
        else
        {
          HalUARTRead (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], LEN_Token - tempDataLen);
          gizPutData(&pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen],LEN_Token - tempDataLen);
